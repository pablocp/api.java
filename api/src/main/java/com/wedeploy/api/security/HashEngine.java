/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under the terms of the
 * GNU Lesser General Public License as published by the Free Software Foundation; either version
 * 2.1 of the License, or (at your option) any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without
 * even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
 * Lesser General Public License for more details.
 */

package com.wedeploy.api.security;

import java.util.HashMap;
import java.util.Map;

/**
 * Main class that performs bindings and holds instances of hash generators.
 */
public class HashEngine {

	/**
	 * Returns singleton instance of the engine.
	 */
	public static HashEngine instance() {
		return instance;
	}

	public static void setFactory(HashGeneratorFactory factory) {
		HashEngine.factory = factory;
	}

	/**
	 * Returns te {@link Parser} instance.
	 */
	public HashGenerator lookup(String algorithm) {
		HashGenerator generator = generators.get(algorithm);

		if (generator != null) {
			return generator;
		}

		if (factory == null) {
			throw new HashEngineException("No hash generator factory set.");
		}

		generator = factory.create(algorithm);
		generators.put(algorithm, generator);

		return generator;
	}

	/**
	 * Manual registration of engines.
	 */
	public HashEngine register(String algorithm, HashGenerator generator) {
		generators.put(algorithm, generator);
		return this;
	}

	protected HashEngine() {
	}

	protected void clearEngine() {
		factory = null;
		generators.clear();
	}

	private static HashGeneratorFactory factory;
	private static final HashEngine instance = new HashEngine();

	private Map<String, HashGenerator> generators = new HashMap<>();

}